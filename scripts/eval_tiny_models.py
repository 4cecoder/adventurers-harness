...
from adventurers_py.tools import view_file, grep_search, replace_file_content, run_command
from adventurers_py.gates import SyntaxGate, DiffGate
...
def run_evaluation_suite() -> List[BenchmarkResult]:
    ...
    vf = view_file(norm_args["path"], start_line=norm_args["start_line"], end_line=norm_args["end_line"])
...
def main():
    console.print(Panel("[bold cyan]🧪 Tiny Model Deterministic Pipeline Objective Benchmark[/bold cyan]\n[dim]Testing grammar repair, AST syntax gates, file slicing, and security filters[/dim]", border_style="cyan"))
    ...
    console.print(Panel(table, title=f"[bold green]Benchmark Results: {total_passed}/{len(results)} Passed ({(total_passed/len(results))*100:.0f}% Score)[/bold green]", border_style="green"))
