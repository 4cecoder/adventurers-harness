...
from adventurers_py.docker_manager import get_docker_status_summary, start_docker_compose, stop_docker_compose
...
def print_docker_status():
    ...
    table.add_row(
        ...
        "Binary detected in PATH" if status["docker_installed"] else "Install Docker Desktop or Colima"
    )
    table.add_row(
        "Docker Daemon",
    ...
    table.add_row(
        ...
        f"[green]{hin['status'].upper()}[/green]" if hin["running"] else "[dim]Offline (Using Native OKF)[/dim]",
    ...
    console.print(Panel(table, title="[bold cyan]🐳 Local Docker & Hindsight Memory Status[/bold cyan]", border_style="cyan"))
...
def main():
    ...
    parser.add_argument("--docker", action="store_true", help="Check status of local Docker containers and Hindsight memory")
    parser.add_argument("--docker-up", action="store_true", help="Start local Docker Compose services")
    parser.add_argument("--docker-down", action="store_true", help="Stop local Docker Compose services")
    ...
    if args.docker_up:
        ...
        if res["success"]:
            console.print("[bold green]✔ Docker Compose services started successfully![/bold green]")
        else:
            console.print(f"[bold red]❌ Failed to start Docker Compose: {res.get('error') or res.get('output')}[/bold red]")
        print_docker_status()
        return
