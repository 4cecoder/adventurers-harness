...
HINDSIGHT_PORT = 8888
COMPOSE_FILE = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "docker-compose.yml")
    ...
    try:
        with httpx.Client(timeout=1.0) as client:
            ...
            if resp.status_code == 200:
                data = resp.json()
                return {"running": True, "status": data.get("status", "healthy"), "service": data.get("service", "hindsight-local")}
...
def get_docker_status_summary() -> Dict[str, Any]:
    ...
    return {
        ...
        "active_memory_mode": "docker_hindsight" if hindsight_info["running"] else "native_on_device_okf"
    }
