...
class WorkloadDomain(str, Enum):
    LOW_LATENCY_COMPUTE = "low_latency_compute"       # Tokenizers, SIMD math, byte parsing, zero-alloc buffers
    MACOS_NATIVE_SYSTEM = "macos_native_system"       # PTY, Metal shaders, AppKit/SwiftUI, OS permissions
    AGENT_HARNESS_GLUE = "agent_harness_glue"         # Deterministic synthesis loops, eval harnesses, test runners
...
LANGUAGE_DECISION_MATRIX: Dict[WorkloadDomain, LanguageRecommendation] = {
    WorkloadDomain.LOW_LATENCY_COMPUTE: LanguageRecommendation(
        ...
        strengths=["Deterministic zero-alloc memory", "Direct POSIX/C ABI interop", "Raw hardware execution speed"],
    ...
    WorkloadDomain.MACOS_NATIVE_SYSTEM: LanguageRecommendation(
        ...
        strengths=["Swift 6 strict concurrency checks", "Direct AppKit/Foundation/PTY bindings", "Zero-copy Metal UMA buffers"],
    ...
    WorkloadDomain.AGENT_HARNESS_GLUE: LanguageRecommendation(
        ...
        strengths=["Sub-second test execution with uv/pytest", "Dynamic code evaluation & string manipulation", "Rich interactive CLI visualizers"],
    ...
    WorkloadDomain.NETWORK_SECURITY: LanguageRecommendation(
        ...
        strengths=["Guaranteed thread safety & memory safety", "Safe byte stream parsing", "Production-grade async Tokio runtime"],
    ...
    WorkloadDomain.DEVOPS_ORCHESTRATION: LanguageRecommendation(
        ...
        strengths=["Native OS shell integration", "Standard Unix pipeline composition", "Universal portability across mac/linux"],
...
def classify_prompt_domain(prompt: str) -> WorkloadDomain:
    ...
    if any(k in prompt_lower for k in ("docker", "compose", "git worktree", "submodule", "sh script", "bash")):
        return WorkloadDomain.DEVOPS_ORCHESTRATION
    elif any(k in prompt_lower for k in ("metal", "uma", "appkit", "swiftui", "macos", "pty", "sendable")):
        return WorkloadDomain.MACOS_NATIVE_SYSTEM
    elif any(k in prompt_lower for k in ("simd", "zero-alloc", "c code", "zig", "byte parser", "bit packing", "1-bit")):
