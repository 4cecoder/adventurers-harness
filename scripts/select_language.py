...
from adventurers_py.language_selector import LANGUAGE_DECISION_MATRIX, classify_prompt_domain, WorkloadDomain
...
def main():
    ...
    console.print(Panel(table, title="[bold green]Polyglot Language Selection Strategy[/bold green]", border_style="green"))
    ...
    if len(sys.argv) > 1:
        ...
        console.print(f"[bold]Recommended Language:[/bold] [bold cyan]{rec.selected_language.value}[/bold cyan]")
        console.print(f"[bold]Verification Step:[/bold] [yellow]{rec.verification_methodology}[/yellow]\n")
